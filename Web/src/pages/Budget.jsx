...
function Budget() {
  ...
  const {setAllocatedBudget, budgetsEdited, setBudgetsEdited, totalBudget, setTotalBudget, potentialTotalBudget, setPotentialTotalBudget, categoryToBudgetDictionary, setCategoryToBudgetDictionary, potentialCategoryToBudgetDictionary, setPotentialCategoryToBudgetDictionary, newCategoryAdded} = useBudget();
  ...
  useEffect(() => {
      const getBudgetDetailsCall = async () => {
          ...
          if (!response.ok) {
              ...
              setPotentialCategoryToBudgetDictionary(null);
              return;
          }
    
          console.log("Found budget details for user: ", data.budgetDetails);
          console.log("Found budget total for user: ", data.budgetTotal);

          setTotalBudget(data.budgetTotal);
          setPotentialTotalBudget(data.budgetTotal);
          setCategoryToBudgetDictionary(data.budgetDetails);
          setPotentialCategoryToBudgetDictionary(data.budgetDetails);
      };
    
      getBudgetDetailsCall();
  }, [newCategoryAdded, budgetsEdited]); //only executes when a new category gets added to the database to reduce calls
  ...
  const handleCloseModal = () => {
      ...
      setPotentialCategoryToBudgetDictionary(categoryToBudgetDictionary);
  };
  ...
  const handleApplyChanges = async() => {
      ...
      const { data: { user } } = await supabase.auth.getUser();
      const userID = user?.id || '';
      const response = await fetch(`${import.meta.env.VITE_API_URL}/change_budgets`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
          },
          body: JSON.stringify({ userID, sbAccessToken, newCategoryToBudgetDictionary: potentialCategoryToBudgetDictionary, newTotal: potentialTotalBudget }),
      });

      const data = await response.json();

      if (!response.ok) {
          console.log("Error occurred trying to edit budgets: ", data.message);
          return;
      }

      console.log("Successfully edited budgets")
      setBudgetsEdited((prev) => `${prev}a`);
    ...
    <Box width="100%">
      ...
      <Box bg={bgColor} p={6} borderRadius="lg" boxShadow="sm">
        ...
        <SimpleGrid columns={{ base: 1, md: 2 }} spacing={6}>
          {categoryToBudgetDictionary ? (Object.entries(categoryToBudgetDictionary).map(([category, budget]) => {
          ...
          <ModalContent>
              ...
              <ModalBody>
                  ...
                  {categoryToBudgetDictionary ? (
                      Object.entries(categoryToBudgetDictionary).map(([category, budget]) => {
